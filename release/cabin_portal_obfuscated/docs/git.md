...
- **Release Secrets**: Local config files inside generated releases (`release/**/config.local.php`).

The generated obfuscated release package under `release/cabin_portal_obfuscated/` and `release/cabin_portal_obfuscated.zip` is intentionally committed. `deploy.php` installs that release package when the Staff Dashboard update button is authorized.
