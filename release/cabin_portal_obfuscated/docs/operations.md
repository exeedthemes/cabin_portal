...
Both are committed to Git so the admin update button can install the obfuscated build from the GitHub archive. The builder does not package `config.local.php`; production keeps its existing local secret during updates.
